...
class TaxiClientToDatabase:
    ...
    def set_TAXI_client(self):
        ...
        client.set_auth(username = self.usernameKey, password = self.userPassword, verify_ssl=False)
        return client

    #Poll content for AlientVault Taxi Server
    def get_STIX_and_insert_database(self):
        Ultility.print_and_write_to_log("-------------------- HOST URL: "+self.hostURL+"----------------------\n")
        #Ingnore Warning
        warnings.filterwarnings("ignore")
        for collectionName in self.collectionNames:
            
            Ultility.print_and_write_to_log("-------------------- Collection Name: "+collectionName+"----------------------\n")
            ...
            for block in content_blocks:
                try:
                    ...
                    domains = Ultility.get_domain_from_XML(block.content,self.hostURL,collectionName)
