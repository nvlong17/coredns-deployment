...
class Ultility:
        ...
        if collectionName == 'ei.botnet':
            ...
            for observable in stixDict['observables']['observables']:
                ...
                for relateObj in observable['object']['related_objects']:
                    if Ultility.is_acceptable_data_type(
                        relateObj['properties']['xsi:type']
                    ):
    ...
    @staticmethod
    def get_domain_from_text(hostURL):
        try:
            rawDomain = requests.get(url=hostURL).content.decode().rstrip().split("\n")
            domains = []
            for domain in rawDomain:
                domains.append(Ultility.get_converted_domain(domain))
            return domains
        except:
            return []
